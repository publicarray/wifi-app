...
//
// The function checks for known WiFi capability strings:
//   - WiFi7 or 802.11be -> "7"
//   - WiFi6 or HE (802.11ax) -> "6"
//   - WiFi5 or VHT (802.11ac) -> "5"
//   - WiFi4, HT, or 802.11n -> "4"
//   - Legacy, 802.11a/b/g -> "Unknown"
func deriveWiFiGeneration(capabilities []string) string {
	...
	for _, cap := range capabilities {
		...
		if strings.Contains(lower, "wifi7") || strings.Contains(lower, "802.11be") {
			return "7"
		}
		if strings.Contains(lower, "eht") || strings.Contains(lower, "802.11be") {
			return "7"
		}
	}
	...
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "wifi6") || strings.Contains(lower, "he") || strings.Contains(lower, "802.11ax") {
			return "6"
		}
	}
	...
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "wifi5") || strings.Contains(lower, "vht") || strings.Contains(lower, "802.11ac") {
			return "5"
		}
	}
	...
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "wifi4") || strings.Contains(lower, "ht") || strings.Contains(lower, "802.11n") {
			return "4"
		}
	}
...
//   - WiFi 7 (802.11be)
//   - WiFi 6E (802.11ax with 6GHz band)
//   - WiFi 6 (802.11ax)
//   - WiFi 5 (802.11ac)
//   - WiFi 4 (802.11n)
//   - Legacy (802.11a/b/g)
func getDominantWiFiStandard(capabilities []string, band string) string {
	...
	// Check for WiFi 7 (EHT/802.11be)
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "eht") || strings.Contains(lower, "wifi7") {
			return "WiFi 7 (802.11be)"
		}
	}

	// Check for WiFi 6E (HE/802.11ax with 6GHz band)
	hasHE := false
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "he") || strings.Contains(lower, "wifi6") || strings.Contains(lower, "802.11ax") {
			hasHE = true
			break
		}
	}
	...
	// Check for WiFi 6 (HE/802.11ax)
	if hasHE {
		return "WiFi 6 (802.11ax)"
	}

	// Check for WiFi 5 (VHT/802.11ac)
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "vht") || strings.Contains(lower, "wifi5") || strings.Contains(lower, "802.11ac") {
			return "WiFi 5 (802.11ac)"
		}
	}

	// Check for WiFi 4 (HT/802.11n)
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "ht") || strings.Contains(lower, "wifi4") || strings.Contains(lower, "802.11n") {
			return "WiFi 4 (802.11n)"
		}
	}

	// Check for Legacy standards
	for _, cap := range capabilities {
		lower := strings.ToLower(strings.TrimSpace(cap))
		if strings.Contains(lower, "legacy") || strings.Contains(lower, "802.11a") ||
			strings.Contains(lower, "802.11b") || strings.Contains(lower, "802.11g") {
...
//   - MU-MIMO (multi-user MIMO implies beamforming)
//   - VHT or HE standards (which support beamforming)
